...
interface Props {
	...
	setListDepartment: Dispatch<SetStateAction<Department[] | []>>;
}
...
const EditarDepartments: React.FC<Props> = ({ listDepartment, setListDepartment }) => {
	...
	const handlesUpdateDepartment = async () => {
		...
		if (listDepartment.length) {
			const res = await seguridad.updateDepartments(listDepartment);
			if (res.ok) {
	...
	const handleCreateDepartment = async () => {
		...
		if (department && listDepartment.length) {
			const res = await seguridad.createDepartment(department);
			if (res.ok) {
